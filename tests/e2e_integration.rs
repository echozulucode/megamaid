...
fn test_complete_workflow_scan_to_execution() {
    ...
    fs::write(
        temp.path().join("project/target/debug/app.exe"),
        vec![0u8; 5_000_000],
    )
    .unwrap();
    fs::write(
        temp.path().join("data/large_file.dat"),
        vec![0u8; 150_000_000],
    )
    .unwrap();
    ...
    assert!(
        detections.len() >= 2,
        "Should detect at least 2 cleanup candidates"
    );
    ...
    assert!(
        plan.entries.len() >= 2,
        "Plan should have at least 2 entries"
    );
    ...
    assert!(
        verification.is_safe_to_execute(),
        "Plan should be safe to execute"
    );
...
fn test_workflow_with_parallel_execution() {
    ...
    assert_eq!(
        file_detections.len(),
        100,
        "Should detect exactly 100 files"
    );
    ...
    assert_eq!(
        ...
        "Should delete all 100 files"
    );
    assert_eq!(result.summary.failed, 0);
    assert!(
        result.summary.space_freed >= 100_000_000,
        "Should free ~100MB"
    );
...
fn test_workflow_with_backup_mode() {
    ...
    assert!(
        backup_dir.join("project/target").exists(),
...
fn test_workflow_with_drift_detection() {
    ...
    assert!(!verification.drifted.is_empty(), "Should report drift");
}
...
fn test_workflow_resilience_to_missing_files() {
    ...
    let mut engine = DetectionEngine::empty();
    engine.add_rule(Box::new(SizeThresholdRule { threshold_bytes: 1 }));
    ...
    assert!(
        total_processed <= plan.delete_count(),
        "Should process files"
    );
    assert!(
        result.summary.successful >= 8,
        "Should delete at least 8 remaining files"
    );
}
